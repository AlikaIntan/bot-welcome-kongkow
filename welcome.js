...
module.exports = function (client) {
    ...
    client.on("guildMemberAdd", async member => {
     ...
     // ctx.fillText(textString2, 730, canvas.height / 2 + 58);
      ...
      ctx.font = 'bold 60px Genta';
      ctx.fillStyle = '#f2f2f2';
      ctx.fillText(textString4, 700, canvas.height / 2 - 100);
...
- Verify your account to access voice channel in the server:
<#775612883203653632> 

- Assign yourself a few roles to finalize your arrival here:
<#774575881309650965>

Have fun at the server !`)
     ...
     { 
      ...
      const ctx = canvas.getContext('2d');
      //set the Background to the welcome.png
      const background = await Canvas.loadImage(`./kongkow.gif`);
      ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
      ctx.strokeStyle = '#f2f2f2';
      ctx.strokeRect(0, 0, canvas.width, canvas.height);
  }
